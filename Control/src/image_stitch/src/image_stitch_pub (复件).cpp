...
int main(int argc, char **argv)
{
  ...
  cap1 >> initial_frame_1;
  cap2 >> initial_frame_2;
  ...
  while (run_flag)
	{
      if (cap1.read(img_left) && cap2.read(img_right))
      {
        t = getTickCount();
        warpPerspective(img_left, imageTransform1, homo, Size(MAX(corners.right_top.x, corners.right_bottom.x), initial_frame_2.rows));

        int dst_width = imageTransform1.cols;  //取最右点的长度为拼接图的长度
        int dst_height = img_right.rows;
      
        Mat dst(dst_height, dst_width, CV_8UC3);
        dst.setTo(0);
      
        imageTransform1.copyTo(dst(Rect(0, 0, imageTransform1.cols, imageTransform1.rows)));
        img_right.copyTo(dst(Rect(0, 0, img_right.cols, img_right.rows)));
      
        imshow("b_dst", dst);

        OptimizeSeam(img_right, imageTransform1, dst);
